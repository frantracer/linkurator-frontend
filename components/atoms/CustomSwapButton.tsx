export enum CustomSwapButtonIcon {
  EyeSlash = "eye-slash",
...
type CustomSwapButtonProps = {
  icon: CustomSwapButtonIcon
  onChecked: () => void
  onUnchecked: () => void
  defaultChecked: boolean
}

export const CustomSwapButton = (props: CustomSwapButtonProps) => {
  return (
    <label className="swap swap-rotate">
      <input onChange={(e) => {
        e.target.checked ? props.onChecked() : props.onUnchecked()
      }} type="checkbox" defaultChecked={props.defaultChecked}/>
      {IconToSvg[props.icon]['on']}
      {IconToSvg[props.icon]['off']}
    </label>
  )
}

const IconToSvg = {
  [CustomSwapButtonIcon.CheckCircle]: {
    on:
  ...
  [CustomSwapButtonIcon.ThumbsDown]: {
    on:
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="swap-on w-6 h-6">
        <path
